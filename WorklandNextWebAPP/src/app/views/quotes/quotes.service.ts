...
export class QuotesService extends BaseService {
  ...
  model = 'PreventivoTestata';
  modelDettaglio = 'PreventivoDettaglio';
  ...
  _getQuoteDefaultValues(aziendaId) {
    let headers = new HttpHeaders();
    headers = headers.set('tenant', this.tenant);
  ...
  getQuoteJsonByDocument(dId: number, tDId: number, mTD: number, aId: number) {
    let httpParams = new HttpParams();
