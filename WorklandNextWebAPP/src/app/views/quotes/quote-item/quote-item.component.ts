...
import { QuoteItem } from '@app/models/quote-item.model';
...
export class QuoteItemComponent implements OnInit, OnChanges {
    ...
    Object.keys(data).forEach((key) => {
      ...
      switch (key) {
        case 'preventivoTestataId':
          value = data[key] ? data[key] : this.quoteId;
          break;
        default:
  ...
  loadQuoteItem() {
    this.loading = true;
    if (this.itemId) {
      this.quotesService.getQuoteItem(this.itemId).subscribe(
        (response: any) => {
          const _quote = this.gridUtils.renameJson(response);
          this.item = new QuoteItem(_quote);
  ...
  loadArticoli() {
    this.articoli$ = concat(
      of([]), // default items
      this.articoliInput$.pipe(
        filter(res => {
          return res !== null && res.length >= this.minLengthTerm;
        }),
        distinctUntilChanged(),
        debounceTime(800),
        tap(() => this.articoliLoading = true),
        switchMap(term => {
          return this.quotesService.getArticoliForAutocomple(term).pipe(
            catchError(() => of([])), // empty list on error
            tap(() => this.articoliLoading = false)
          );
        })
      )
    );
