...
@Component({
  selector: 'app-order-supplier-quotes',
  templateUrl: 'order-supplier-quotes.component.html',
  styleUrls: ['./order-supplier-quotes.component.scss']
})
export class OrderSupplierQuoteComponent implements OnInit {
  ...
  initData() {
    ...
    this.quotesService.getGrid('ListaOrdiniClienteCollegati').subscribe(
      (response: any) => {
  ...
  loadQuotes() {
    this.loading = true;
    this.quotesService.getQuoteJsonByDocument(this.orderId, this.tipoDocumentoId, this.macroTipoDocumento, this.aziendaId).subscribe(
      (response: any) => {
