...
export class OrdersSuppliersService extends BaseService {

  model = 'OrdineFornitoreTestata';
  modelDettaglio = 'OrdineFornitoreDettaglio';
  ...
  _getOrderDefaultValues(aziendaId) {
    let headers = new HttpHeaders();
    headers = headers.set('tenant', this.tenant);

    let httpParams = new HttpParams();
    httpParams = httpParams.set('nomeSp', 'SpGetDefaultOrdineFornitoreTestata');
  ...
  saveOrder(id, body) {
    ...
    body.statoDocumentoId = parseInt(body.statoDocumentoId, 10);
    body.contattoDestinazioneId = parseInt(body.contattoDestinazioneId, 10);
    body.vettoreId = parseInt(body.vettoreId, 10);
  ...
  getOrderDetails(id, charge = false) {
    let httpParams = new HttpParams();
    ...
    if (charge) {
      return this.http.get<any[]>(`${this.apiUrl}/${this.modelDettaglio}/CreaCarico`, this.httpOptions);
    } else {
  ...
  chargeWarehouse(body) {
    ...
    return this.http.post(`${this.apiUrl}/${this.modelDettaglio}/ConfermaCarico`, body, this.httpOptions);
  }

  getOrderSuppliersJsonByDocument(dId: number, tDId: number, mTD: number, aId: number) {
    let httpParams = new HttpParams();
