...
@Component({
  selector: 'app-contact-orders-suppliers',
  templateUrl: 'contact-orders-suppliers.component.html',
  styleUrls: ['./contact-orders-suppliers.component.scss']
})
export class ContactOrdersSuppliersComponent implements OnInit {

  @Input() contactId = null;

  model = 'orders-suppliers';
  ...
  initData() {
    ...
    this.ordersService.getGrid('ListaOrdiniFornitoreCollegati').subscribe(
      (response: any) => {
  ...
  loadOrders() {
    this.loading = true;
    this.ordersService.getOrdersContact(this.contactId).subscribe(
      (response: any) => {
        const _result = response;
        this.orders = _result ? _result.map(item => this.gridUtils.renameJson(item)) : [];
        this.ordersMeta = _result ? _result.meta : null;
