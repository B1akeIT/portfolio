...
export class InvoiceItemComponent implements OnInit, OnChanges {
  ...
  initForm(data) {
    Object.keys(data).forEach((key) => {
      let value = '';
      switch (key) {
        case 'fatturaTestataId':
          value = data[key] ? data[key] : this.invoiceId;
          break;
        default:
    ...
    if (this.itemId) { this.ricalcolaValori('all'); }
  ...
  loadInvoiceItem() {
    this.loading = true;
    if (this.itemId) {
      this.invoicesService.getInvoiceItem(this.itemId).subscribe(
        (response: any) => {
          const _item = this.gridUtils.renameJson(response);
          console.log('_item', _item);
          this.item = new InvoiceItem(_item);
      ...
      this.item = new InvoiceItem({
        fatturaTestataId: this.invoiceId,
