...
export class WarehouseMovementsService extends BaseService {

  model = 'DocumentoDiTrasportoTestata';
  modelDettaglio = 'DocumentoDiTrasportoDettaglio';
  ...
  _getDdtDefaultValues(aziendaId: number, tipoDocumentoId: number) {
    let headers = new HttpHeaders();
    headers = headers.set('tenant', this.tenant);

    let httpParams = new HttpParams();
    httpParams = httpParams.set('nomeSp', 'SpGetDefaultDocumentoDiTrasportoTestata');
  ...
  getDocumentsJsonByDocument(dId: number, tDId: number, mTD: number, aId: number) {
    let httpParams = new HttpParams();
