...
@Component({
  selector: 'app-invoice-warehouse-movements',
  templateUrl: 'invoice-warehouse-movements.component.html',
  styleUrls: ['./invoice-warehouse-movements.component.scss']
})
export class InvoiceWarehouseMovementsComponent implements OnInit {
  ...
  constructor(
    ...
    private warehouseMovementsService: WarehouseMovementsService,
  ...
  ngOnInit(): void {
    ...
    this.warehouseMovementsService.setTenent(tenant);
    this.initData();
  }
  ...
  initData() {
    ...
    this.warehouseMovementsService.getGrid('ListaPreventiviCollegati').subscribe(
      (response: any) => {
  ...
  onFilterChanged(event) {
    this.warehouseMovementsService.setQuery(event);
    this.loadWarehouseMovements();
  }

  loadWarehouseMovements() {
    this.loading = true;
    this.warehouseMovementsService.getDocumentsJsonByDocument(this.invoiceId, this.tipoDocumentoId, this.macroTipoDocumento, this.aziendaId).subscribe(
      (response: any) => {
        const _result = response.items;
        this.warehouseMovementsService = _result ? _result.map(item => this.gridUtils.renameJson(item)) : [];
        this.warehouseMovementsMeta = response.meta || null;
        if (this.warehouseMovementsMeta && this.warehouseMovementsMeta.totalItems) {
          this.numPages = Math.ceil(this.warehouseMovementsMeta.totalItems / this.perPage);
          const fromCount = (this.currentPage - 1) * this.perPage + 1;
          const toCount = this.isLastPage() ? this.warehouseMovementsMeta.totalItems : this.currentPage * this.perPage;
          const count = fromCount + ' - ' + toCount;
          this.countTranslateParams = {
            count: count,
            total: this.warehouseMovementsMeta.totalItems
          };
        } else {
          this.numPages = 0;
          const count = String(this.warehouseMovements.length);
          const total = String(this.warehouseMovements.length);
  ...
  resetPagination() {
    ...
    this.currentPage = 1;
    this.warehouseMovementsService.setLimit(this.limit);
    this.warehouseMovementsService.setPerPage(this.perPage);
    this.warehouseMovementsService.setPage(this.currentPage);
    this.warehouseMovementsService.setFilterSearch(this.filterData);
    this.warehouseMovementsService.setFilterQuery(this.query);
    this.warehouseMovementsService.setSort(this.sort.column, this.sort.direction);
    this.warehouseMovementsService.setDateRange(this.filterRangeDate);
    this.clearCurrent();
  }

  isLastPage() {
    if (this.warehouseMovementsMeta) {
      return (this.currentPage * this.perPage > this.warehouseMovementsMeta.totalItems);
    }
    return false;
  }
  ...
  onSortChanged(event) {
    ...
    if (sortState.length === 0) {
      this.warehouseMovementsService.setSort(this.warehouseMovementsService.sortDefault.column, this.warehouseMovementsService.sortDefault.direction);
    } else {
      const item = sortState[0];
      this.warehouseMovementsService.setSort(item.colId, item.sort);
    }
    this.loadWarehouseMovements();
  }
  ...
  onGroupSorting($event) {
    ...
    this.warehouseMovementsService.setSort($event.field, $event.direction);
