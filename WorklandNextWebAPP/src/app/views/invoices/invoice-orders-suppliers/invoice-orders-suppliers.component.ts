...
@Component({
  selector: 'app-invoice-orders-suppliers',
  templateUrl: 'invoice-orders-suppliers.component.html',
  styleUrls: ['./invoice-orders-suppliers.component.scss']
})
export class InvoiceOrdersSuppliersComponent implements OnInit {
  ...
  constructor(
    ...
    private ordersSuppliersService: OrdersSuppliersService,
  ...
  ngOnInit(): void {
    ...
    this.ordersSuppliersService.setTenent(tenant);
    this.initData();
  }
  ...
  initData() {
    ...
    this.ordersSuppliersService.getGrid('ListaOrdiniFornitoreCollegati').subscribe(
      (response: any) => {
  ...
  onFilterChanged(event) {
    this.ordersSuppliersService.setQuery(event);
    this.loadOrders();
  }

  loadOrders() {
    this.loading = true;
    this.ordersSuppliersService.getOrderSuppliersJsonByDocument(this.invoiceId, this.tipoDocumentoId, this.macroTipoDocumento, this.aziendaId).subscribe(
      (response: any) => {
  ...
  resetPagination() {
    ...
    this.currentPage = 1;
    this.ordersSuppliersService.setLimit(this.limit);
    this.ordersSuppliersService.setPerPage(this.perPage);
    this.ordersSuppliersService.setPage(this.currentPage);
    this.ordersSuppliersService.setFilterSearch(this.filterData);
    this.ordersSuppliersService.setFilterQuery(this.query);
    this.ordersSuppliersService.setSort(this.sort.column, this.sort.direction);
    this.ordersSuppliersService.setDateRange(this.filterRangeDate);
    this.clearCurrent();
  }
  ...
  onSortChanged(event) {
    ...
    if (sortState.length === 0) {
      this.ordersSuppliersService.setSort(this.ordersSuppliersService.sortDefault.column, this.ordersSuppliersService.sortDefault.direction);
    } else {
      const item = sortState[0];
      this.ordersSuppliersService.setSort(item.colId, item.sort);
    }
    this.loadOrders();
  }
  ...
  onGroupSorting($event) {
    ...
    this.ordersSuppliersService.setSort($event.field, $event.direction);
