...
export class InvoiceOrdersComponent implements OnInit {
  ...
  initData() {
    ...
    this.invoicesService.getGrid('ListaOrdiniClienteCollegati').subscribe(
      (response: any) => {
  ...
  loadOrders() {
    this.loading = true;
    this.ordersService.getOrderJsonByDocument(this.invoiceId, this.tipoDocumentoId, this.macroTipoDocumento, this.aziendaId).subscribe(
      (response: any) => {
