...
export class InvoicesComponent implements OnInit {
  ...
  invoice: Invoice;
  invoiceFields: Array<FormlyFieldConfig>;
  ...
  constructor(
    ...
    this.eventsManagerService.on(APP_CONST.invoiceUpdateEvent, (event) => {
      const invoice = event;
      const index = _.findIndex(this.invoices, { id: invoice.id });
      this.setNewData(invoice.id, invoice);
    });
  }
  ...
  ngOnInit(): void {
    ...
    this.invoicesService.setAziendaId(this.currentCompanyId);
    ...
    this.invoicesService.setTenent(this.tenant);
    this.initData();
  ...
  initData() {
    ...
    this.invoicesService.getGrid('RicercaFatturaTestataView').subscribe(
      (response: any) => {
  ...
  loadInvoices() {
    this.loading = true;
    // this.invoicesService.getJson(this.model)
    this.invoicesService.getListModel().subscribe(
      (response: any) => {
        this.invoices = (response.items || []).map(item => this.gridUtils.renameJson(item));
        this.invoicesMeta = response.meta;
  ...
  onSave(event) {
    if (event.isNew) { this.loadInvoices(); }
  }

  onDelete(confirm) {
    if (confirm) {
      this.invoicesService.deleteModel(this.currentId).subscribe(
        (response: any) => {
          this.notificationService.success('APP.MESSAGE.deletion_successful', 'APP.TITLE.invoices');
          this.refresh();
        },
        (error: any) => {
          console.log('onDelete error', error);
          this.notificationService.error('APP.MESSAGE.deletion_unsuccessful', 'APP.TITLE.invoices');
        }
      );
    } else {
      this.notificationService.info('APP.MESSAGE.deletion_canceled', 'APP.TITLE.invoices');
    }
  }
  ...
  onEdit(isNew = false, invoiceType = null) {
    if (isNew) {
      this.clearCurrent();
    }

    this.newInvoiceType = invoiceType;
  ...
  togglePanel() {
    ...
    this.currentInvoice = this.panelIsOpen ? this.currentElement : null;
  }
  ...
  clearFilterSort() {
    ...
    if (oneEmit) {
      ...
      // this.invoicesFilterComponent.clearFilter(true);
    }
  }
  ...
  onGroupFilters($event) {
    ...
    //   }
    // );
    // this.invoicesService.setFilterSearch(filter);
