...
@Component({
  selector: 'app-ddt-quotes',
  templateUrl: 'ddt-quotes.component.html',
  styleUrls: ['./ddt-quotes.component.scss']
})
export class DdtQuoteComponent implements OnInit {
  ...
  loadQuotes() {
    this.loading = true;
    this.quotesService.getQuoteJsonByDocument(this.ddtId, this.tipoDocumentoId, this.macroTipoDocumento, this.aziendaId).subscribe(
      (response: any) => {
        const _result = response.items;
        this.quotes = _result ? _result.map(item => this.gridUtils.renameJson(item)) : [];
        this.quotesMeta = response.meta || null;
