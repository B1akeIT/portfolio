...
@Component({
  selector: 'app-ddt-documents',
  templateUrl: 'ddt-documents.component.html',
  styleUrls: ['./ddt-documents.component.scss']
})
export class DdtDocumentsComponent implements OnInit {
        ...
        if (response && response.schemaDescription) {
          ...
          if (visibilityStatus !== 0) {
            this.schema.sort((a: any, b: any) => {
              const aObj = _.find(ddtColumns, { 'columnName': a.ColumnName });
              const bObj = _.find(ddtColumns, { 'columnName': b.ColumnName });
  ...
  loadDocuments() {
    this.loading = true;
    this.ddtService.getDocumentsJsonByDocument(this.ddtId, this.tipoDocumentoId, this.macroTipoDocumento, this.aziendaId).subscribe(
      (response: any) => {
        const _result = response.items;
        this.documents = _result ? _result.map(item => this.gridUtils.renameJson(item)) : [];
        this.documentsMeta = response.meta || null;
        if (this.documentsMeta && this.documentsMeta.totalItems) {
          this.numPages = Math.ceil(this.documentsMeta.totalItems / this.perPage);
          const fromCount = (this.currentPage - 1) * this.perPage + 1;
          const toCount = this.isLastPage() ? this.documentsMeta.totalItems : this.currentPage * this.perPage;
          const count = fromCount + ' - ' + toCount;
          this.countTranslateParams = {
            count: count,
            total: this.documentsMeta.totalItems
          };
        } else {
          this.numPages = 0;
          const count = String(this.documents.length);
          const total = String(this.documents.length);
  ...
  isLastPage() {
    if (this.documentsMeta) {
      return (this.currentPage * this.perPage > this.documentsMeta.totalItems);
    }
    return false;
  }
