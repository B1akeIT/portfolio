...
import { ModalDdtItemComponent } from '@app/views/ddt/modal-ddt-item/modal-ddt-item.component';
...
import { DdtItem } from '../../../models/ddt-item.model';
...
@Component({
  selector: 'app-ddt-body',
  templateUrl: './ddt-body.component.html',
  styleUrls: ['./ddt-body.component.scss']
})
export class DdtBodyComponent implements OnInit, OnChanges {

  @Input() ddtId: number | null = null;
  ...
  ngOnInit() {
    ...
    this.ddtService.getGrid('RicercaDocumentoDiTrasportoDettaglioView').subscribe(
      (response: any) => {
  ...
  loadDdtDetails() {
    this.loading = true;
    this.clearCurrent();
    this.ddtService.getDdtDetails(this.ddtId, this.charge).subscribe(
      (response: any) => {
        if (this.charge) {
  ...
  onQuickAdd() {
    this.buildQuickForm();
    this.quickData = new DdtItem({
      documentoDiTrasportoTestataId: this.ddtId,
  ...
  onQuickSave(values: any) {
    ...
    if (this.quickData.quantita > 0) {
      ...
      this.ddtService.saveArticolo(0, body).subscribe(
        ((response: any) => {
  ...
  onDelete() {
    if (this.currentItem) {
      this.ddtService.deleteDdtItem(this.currentItem.id).subscribe(
        (response) => {
  ...
  openModal(isNew = false) {
    ...
    this.modalRef = this.modalService.show(ModalDdtItemComponent, {
