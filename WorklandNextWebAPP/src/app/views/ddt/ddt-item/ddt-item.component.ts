...
import { DdtItem } from '@app/models/ddt-item.model';
...
@Component({
  selector: 'app-ddt-item',
  templateUrl: './ddt-item.component.html',
  styleUrls: ['./ddt-item.component.scss']
})
export class DdtItemComponent implements OnInit, OnChanges {
    ...
    Object.keys(data).forEach((key) => {
      ...
      switch (key) {
        case 'ordineClienteTestataId':
          value = data[key] ? data[key] : this.ddtId;
          break;
        default:
  ...
  loadDdtItem() {
    this.loading = true;
    if (this.itemId) {
      this.ddtService.getDdtItem(this.itemId).subscribe(
        (response: any) => {
          const _ddt = this.gridUtils.renameJson(response);
          this.item = new DdtItem(_ddt);
      ...
      this.item = new DdtItem({
        preventivoTestataId: this.ddtId,
