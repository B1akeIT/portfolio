...
@Component({
  selector: 'app-ddt-orders',
  templateUrl: 'ddt-orders.component.html',
  styleUrls: ['./ddt-orders.component.scss']
})
export class DdtOrdersComponent implements OnInit {
  ...
  initData() {
    ...
    this.ordersService.getGrid('ListaOrdiniClientiCollegati').subscribe(
      (response: any) => {
  ...
  loadOrders() {
    this.loading = true;
    this.ordersService.getOrderJsonByDocument(this.ddtId, this.tipoDocumentoId, this.macroTipoDocumento, this.aziendaId).subscribe(
      (response: any) => {
