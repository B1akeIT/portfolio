...
import { ModalOrderEditComponent } from '@app/views/orders/modal-order-item/modal-order-item.component';
...
@Component({
  ...
  styleUrls: ['./order-body.component.scss']
})
export class OrderBodyComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    this.ordersService.getGrid('RicercaOrdineClienteDettaglioView').subscribe(
      (response: any) => {
        if (response && response.schemaDescription) {
          ...
          // PaginationStatus { pageSize, orderColumn, orderType, pageNumber}
  ...
  initAgGrid() {
    ...
    this.schema.forEach(obj => {
      ...
      if (cDef) {
        switch (cDef.field) {
          case 'quantita':
            if (this.multiSelection && this.qtaEditable) {
              cDef.editable = true;
              ...
              cDef.cellClass = ['text-center', 'bg-gray-100'];
            }
            break;
        }
        this.agColumnDefs.push(cDef);
  ...
  onQuickAdd() {
    ...
    this.quickData = new OrderItem({
      ordineClienteTestataId: this.orderId,
  ...
  openModal(isNew = false) {
    ...
    this.modalRef = this.modalService.show(ModalOrderEditComponent, {
