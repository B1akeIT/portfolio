import {Component, Input, OnInit, OnDestroy, Inject, Renderer2, HostBinding, OnChanges} from '@angular/core';
...
export class AppHeaderComponent implements OnInit, OnDestroy, OnChanges {
  ...
  ngOnChanges(changes: any) {
    if (changes.navbarBrandFull) {
      this.navbarBrandFull = { ...changes.navbarBrandFull.currentValue };
    }
    if (changes.navbarBrandMinimized) {
      this.navbarBrandMinimized = { ...changes.navbarBrandMinimized.currentValue };
    }
    this.navbarBrandImg = Boolean(this.navbarBrand || this.navbarBrandFull || this.navbarBrandMinimized);
  }
